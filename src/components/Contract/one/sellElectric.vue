      ...
      <div class="ma_content">
        ...
        <div v-if="this.no_val == 1" class="content_list">
          <div
            ...
            :key="index"
            @click="list_daterial(item.id,item.dataSourceType,index)"
...
export default {
  ...
  methods: {
    ...
    list_daterial(id, type, index) {
      ...
      if (type == "INPUT") {
        ...
        this.$router.push({
          name: "showPdf",
          params: { one: this.list_con.list[index].filePath, two: id }
        });
        console.log(
          "http://39.98.43.90/downloads?fileId=" +
            this.list_con.list[index].filePath
        );
      }
    ...
    list_find(data, type) {
      ...
      list_contract.contractListService(
        data,
        res => {
          //合同列表
    ...
    remoteMethod(query) {
      ...
      if (query !== "") {
        ...
        (this.name_list = "{'name': '" + query + "'}"),
          setTimeout(() => {
            this.loading = false;
            list_contract.customerListService(
              this.name_list,
              res => {
                //购电方
                this.$emit("login-success", res);
  ...
  created() {
    ...
    list_contract.contractCodeService(
      this.Service,
      res => {
        //合同字典
        this.$emit("login-success", res);
