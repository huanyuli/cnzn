<template>
  <div>
    <div class="left_two">
      <div v-if="show_map(5) == 5" @click='bb' class="left_two_list" :class="{click_list:this.click_index == 1}">
        <div class="list_right">
          <p>售电合同</p>
        </div>
      </div>
      <div v-if="show_map(6) == 6"  @click='aa' class="left_two_list" :class="{click_list:this.click_index == 2}">
        <div class="list_right list_right_one">
          <p>三方合同</p>
        </div>
      </div>
      <div v-if="show_map(7) == 7"  @click='cc' class="left_two_list" :class="{click_list:this.click_index == 3}">
        <div class="list_right list_right_one">
          <p>购电合同</p>
        </div>
      </div>
...
<script>
  export default {
    ...
    methods: {
      ...
      aa(){
        this.click_index = 2;
        this.$router.push('/Contract/thirdpartyList');
      },
      bb(){
        this.click_index = 1;
...
console.log(this.show_map(5) == 5)
      if(this.show_map(5) == 5){
        this.click_index = 1;
        this.$router.push('/Contract/sellElectric');
      }else if(this.show_map(5) != 5 && this.show_map(6) == 6){
        this.click_index = 2;
        this.$router.push('/Contract/thirdpartyList');
      }else if(this.show_map(5) != 5 && this.show_map(6) != 6 && this.show_map(7) == 7){
        this.click_index = 3;
        this.$router.push('/Contract/purchaseList');
      }
     ...
     // this.$router.push({path:'/Contract/addSucceed',query:{btn_id:2,cont_id:9}});  //成功页面，进入编辑页面
//      this.click_index = 2;
//      this.$router.push('/Contract/thirdpartyList');
