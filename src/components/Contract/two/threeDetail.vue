          ...
          <div class="ma_left">
            ...
            <div class="ma_left_con">
              <div class="list_right">
                <div class="right_div"><span>购</span>{{this.deta_list.customer}}</div>
                <i class="el-icon-arrow-right"></i>
                <div class="right_div"><span>售</span>{{this.deta_list.sale}}</div>
                <i class="el-icon-arrow-right"></i>
                <div class="right_div"><span>供</span>{{this.deta_list.provider}}</div>
              </div>
              <div class="con_div">
                <p>购电方 ： {{this.deta_list.customer}}</p>
  ...
  export default {
    data() {
      return {
        ...
        up_change:0,
        dow_url:"",
        dataurl:this.HOST + "/apiFile/thirdContractEditService ",  //合同编辑
        threeAlert:false,
        fromdata:"",  //列表参数
        fromeRemove:"", //删除参数
        deta_list:{},
    ...
    methods: {
      ...
      dele_remove(){
        this.$confirm('确认删除该合同信息？', '确认删除', {
          ...
          add_ajax.thirdContractDeleteService(this.fromdata, res => {  //
     ...
     //   console.log(file);
      ...
      submitForm(formName) {  //表单提交验证
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            add_ajax.thirdContractEditService(this.up_file, res => {  //合同列表
              this.$emit('login-success', res)
            }, (res) => {
              if(res.status == 200){
      ...
      find_date(){
        this.fromdata = "{'id':"+ this.one.id +"}"
        add_ajax.thirdContractDetailService(this.fromdata, res => {  //
          this.$emit('login-success', res);
        }, (res) => {
          ...
          this.ruleForm.vue9 = this.deta_list.customer;
