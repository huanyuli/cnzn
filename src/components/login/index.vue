...
export default {
  data() {
    return {
      ...
      rules: {
        vue_1: [{ required: true, message: "请输入用户账号", trigger: "blur" }],
        vue_2: [{ required: true, message: "请输入登录密码", trigger: "blur" }]
      }
    };
  },
          ...
          ajax_list.userLoginService(
            ...
            res => {
              if (res.status == 200) {
                ...
                if (res.body.isLeader) {
                  window.localStorage.setItem("ISLEADER", "Y");
                } else {
                  window.localStorage.setItem("ISLEADER", "N");
                }
